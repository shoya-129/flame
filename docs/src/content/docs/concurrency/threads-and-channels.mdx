...
## Channels: `send` & `recv` (Under Development)
...
```
<Aside type="caution" title="thread.channel() is under development">
Note: `thread.channel()` is still under development and may not work properly.
</Aside>
