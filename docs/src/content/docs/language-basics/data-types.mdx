...
You can also combine interpolation with multiline strings using `$"""`:
...
let str = $"""
    ...
    Your score is {score}.
    If you need a literal brace, escape it like \{ or \}.
    """
```
...
```flame
// Parameter closure types (with optional parameter names)
fn onEach(callback: (msg: String)) {
    callback("Hello from Flame!")
}

// Full closure signature with return type
fn compute(op: (a: Int, b: Int) -> Int) -> Int {
    return op(10, 20)
}

// Variable declaration with closure type
let logger: (msg: String) = (msg) {
    println($"[LOG]: {msg}")
}
```
...
<Aside type="tip" title="Implicit Nil Return">
Callbacks and event listeners that do not return a value do not need to specify `-> Nil`. Flame automatically assumes a `Nil` return when the arrow return type is omitted (e.g., `(msg: String)` is equivalent to `(msg: String) -> Nil`).
</Aside>
...
<Aside type="note" title="Future Roadmap: Strict Closure Checking">
Strict compile-time closure signature verification may be introduced in future Flame releases under an optional compiler flag (such as `--strict-types` or `--strict-closures`). The current dynamic model ensures seamless scripting flexibility while still giving you full IDE intelligence.
</Aside>
