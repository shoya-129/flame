...
Converts a string into a dynamic array of UTF-8 byte values (`Value::Bytes`):

```flame
let bytes = "Flame".toBytes() // <Bytes>
```
...
For `Bytes` arrays (such as those returned by `.toBytes()`), Flame provides built-in utilities:
...
```flame
let b1 = "hello".toBytes()
let b2 = " world".toBytes()
...
| `.toBytes()` | `String` | `Bytes` | No |
| `.toHex()` | `Bytes` | `String` | No |
| `.toBase64()` | `Bytes` | `String` | No |
| `.concat(Bytes)` | `Bytes` | `Bytes` | No |
| `.type()` | `Any` | `String` | No |
