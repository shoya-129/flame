...
> [!IMPORTANT]
> **Naming Convention**: Custom annotations *must* begin with an uppercase letter (e.g., `Logger`, not `logger`). The compiler enforces this via a mandatory lint rule to ensure annotations remain easily distinguishable from standard functions.
