...
| **`@Suggestion`**| **IDE** | `@Suggestion(["TypeName", "object"])` | Provides advanced IDE auto-completion suggestions for package variables. Type kind can be `object`, `array`, `function`, or `formula`. |
