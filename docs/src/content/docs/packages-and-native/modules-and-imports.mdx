...
Run `flame install` to resolve and cache dependencies into `.flame/pkg/`:

```bash title="bash"
flame install
```

During `flame build`, Blaze performs dependency analysis and constructs an **application-specific native runtime** containing only the packages and native components your application actually imports and uses.
