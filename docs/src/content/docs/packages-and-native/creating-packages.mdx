...
<Steps>
1. **Create the Rust Plugin**: Run `flame new --plugin <plugin_name>` (or `flame new -p <plugin_name>`) to scaffold a new Rust plugin in the `./native` directory. *(Note: Ensure the plugin name is different from your Flame package name).*
2. **Register it**: Run `flame add --plugin ./native` (or `flame add -p ./native`) to register the plugin in `flame.toml` under `[plugins]`. The plugin name is automatically read from `native/Cargo.toml`.
...
Flame fetches the package natively, caches the Flame sources and `.fmi` interface files, and makes your package functionality instantly available. Consumers **never need to manually copy Rust source code or manage Cargo files in their applications**. During `flame build`, Flame's dependency analysis automatically integrates only the required native plugins into that application's specialized runtime.
