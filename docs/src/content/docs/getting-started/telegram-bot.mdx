...
6. ### Run the Bot
   ...
   ```powershell
   flame run src/main.fm
   ```
   
   <Aside type="caution" title="Security Warning">
   Ensure that you never commit your raw `bot_token` to public repositories. Consider loading it via an environment variable (`std.os.getenv`) in production!
   </Aside>
