...
#### `flame run`
Runs a specific `.fm` source file directly:
```bash
# Run a specific file
flame run src/main.fm

# Run with local plugins and native bridge compilation
flame run src/main.fm --local
```
...
`flame build --release` configures LLVM for maximum performance:
