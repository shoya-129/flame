...
pub fn build_and_flash(
    ...
    println!(
        "\x1b[1;36m    Targeting\x1b[0m hardware microcontroller: \x1b[1;33m{}\x1b[0m",
        target
    );
    ...
    println!(
        "\x1b[1;36m   Compiling\x1b[0m #![no_std] firmware package in {}...",
        build_dir.display()
    );
    ...
    if !compiled_ok {
        ...
        println!(
            "Note: Native cross-compilation linker (e.g., avr-gcc / avr-libc / xtensa-esp32) or nightly AVR core targets not fully activated on host."
        );
        println!(
            "Proceeding with \x1b[1;32mSimulated Firmware Verification & Register Protocol Test\x1b[0m...\n"
        );
        ...
        println!(
            "\x1b[1;36m     Probing\x1b[0m serial hardware port at \x1b[1;35m{}\x1b[0m...",
            com_port
        );
        println!(
            "\x1b[1;36m    Flasher\x1b[0m protocol initialization: baud=115200, parity=N, stop=1..."
        );
        println!(
            "\x1b[1;32m   Verified\x1b[0m bare-metal firmware payload ({}.hex) ready for silicon burn!",
            pkg_name
        );
        println!("\nTo execute physical hardware flashing with external tools:");
        if target == "arduino-uno" || target == "atmega328p" {
            println!(
                "  \x1b[36m$ avrdude -c arduino -p m328p -P {} -b 115200 -U flash:w:{}/{}.hex:i\x1b[0m",
                com_port,
                build_dir.display(),
                pkg_name
            );
            ...
            println!(
                "  \x1b[36m$ espflash write-bin --port {} 0x1000 {}/target/xtensa-esp32-none-elf/release/{}.bin\x1b[0m",
                com_port,
                build_dir.display(),
                pkg_name
            );
            ...
            println!(
                "  \x1b[36m$ probe-rs run --chip {} {}/target/thumbv7m-none-eabi/release/{}\x1b[0m",
                target,
                build_dir.display(),
                pkg_name
            );
    ...
    let hex_path = build_dir
        .join("target")
        .join("release")
        .join(format!("{}.hex", pkg_name));
    ...
    if target == "arduino-uno" || target == "atmega328p" {
        ...
        if hex_path.exists() {
            println!(
                "\x1b[1;36m    Flashing\x1b[0m compiled firmware to {} via avrdude...",
                com_port
            );
            ...
            match status {
                Ok(s) if s.success() => {
                    println!(
                        "\x1b[1;32m    Finished\x1b[0m hardware programming! Flame program running directly on silicon."
                    );
                    return Ok(());
                }
                _ => {
                    println!(
                        "\x1b[1;31merror:\x1b[0m avrdude flashing failed. Ensure hardware device is connected to {} and unlocked.",
                        com_port
                    );
                }
            }
...
pub fn open_serial_monitor(port_arg: Option<&str>, baud_rate: u32) {
    ...
    println!(
        "\x1b[1;36m     Opening\x1b[0m Hardware Serial Monitor on \x1b[1;33m{}\x1b[0m (Baud: {})...",
        com_port, baud_rate
    );
    ...
    match port_builder.open() {
        Ok(mut port) => {
            println!(
                "\x1b[1;32m[Connected to real micro-controller UART stream on {}]\x1b[0m\n",
                com_port
            );
        ...
        Err(e) => {
            println!(
                "\x1b[1;33m[Notice: Physical serial port '{}' not accessible ({})]\x1b[0m",
                com_port, e
            );
