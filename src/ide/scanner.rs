...
pub fn scan_document(content: &str) -> (Vec<ScannedVar>, Vec<ScannedStruct>) {
    ...
    for cap in struct_header_re.captures_iter(content) {
        let doc = cap.get(1).or_else(|| cap.get(2)).map(|m| m.as_str().to_string());
    ...
    let fn_start_re = Regex::new(r#"(?:@Docs\s*\(\s*(?:"([^"]*)"|'([^']*)')\s*\)\s*)?fn\s+([a-zA-Z_]\w*)\s*\("#).unwrap();
    for cap in impl_header_re.captures_iter(content) {
        ...
        if let Some(body) = extract_balanced_block(content, open_brace_pos) {
            ...
            for fn_cap in fn_start_re.captures_iter(body) {
                let m_doc = fn_cap.get(1).or_else(|| fn_cap.get(2)).map(|m| m.as_str().to_string());
                ...
                if let Some(close_pos) = paren_close_pos {
                    ...
                    let ret_str = if let Some(arrow_idx) = after_paren.find("->") {
                        ...
                        let end_pos = after_arrow.find('{').unwrap_or_else(|| after_arrow.find('\n').unwrap_or(after_arrow.len()));
    ...
    let mut push_or_update = |name: String, typ: String, doc: Option<String>| {
    ...
    let on_connect_re = Regex::new(r"onConnect\s*\(\s*(?:fn\s*)?\(\s*([a-zA-Z_]\w*)(?:\s*:\s*[a-zA-Z_]\w*)?\s*\)").unwrap();
    for cap in on_connect_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "ServerClient".to_string(), Some(client_doc.clone()));
    }
    ...
    for cap in on_msg_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "ServerClient".to_string(), Some(client_doc.clone()));
        push_or_update(cap[2].to_string(), "String".to_string(), Some("UTF-8 text message payload received from the WebSocket connection.".to_string()));
    }
    ...
    for cap in on_bin_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "ServerClient".to_string(), Some(client_doc.clone()));
        push_or_update(cap[2].to_string(), "Bytes".to_string(), Some(bytes_doc.clone()));
    }
    ...
    for cap in on_close_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "ServerClient".to_string(), Some(client_doc.clone()));
        push_or_update(cap[2].to_string(), "Int".to_string(), Some("WebSocket close status code according to RFC 6455 (e.g., `1000` Normal Closure, `1001` Going Away, `1006` Abnormal Closure).".to_string()));
        push_or_update(cap[3].to_string(), "String".to_string(), Some("WebSocket close reason message explaining why the connection terminated.".to_string()));
    }
    ...
    for cap in on_err_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "ServerClient".to_string(), Some(client_doc.clone()));
        push_or_update(cap[2].to_string(), "String".to_string(), Some("WebSocket error diagnostic message describing transport or protocol failure.".to_string()));
    }
    ...
    for cap in client_msg_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "String".to_string(), Some("UTF-8 text message payload received from the WebSocket connection.".to_string()));
    }
    ...
    for cap in client_bin_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "Bytes".to_string(), Some(bytes_doc.clone()));
    }
    ...
    for cap in client_close_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "Int".to_string(), Some("WebSocket close status code according to RFC 6455 (e.g. 1000 for normal closure).".to_string()));
        push_or_update(cap[2].to_string(), "String".to_string(), Some("WebSocket close reason message.".to_string()));
    }
    ...
    for cap in client_err_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "String".to_string(), Some("WebSocket error diagnostic message describing transport or protocol failure.".to_string()));
    }

    // Stream callback: stream.forEach((msg) { ... })
    let stream_for_re = Regex::new(r"\.(?:forEach|onEach)\s*\(\s*(?:fn\s*)?\(\s*([a-zA-Z_]\w*)(?:\s*:\s*[a-zA-Z_]\w*)?\s*\)").unwrap();
    for cap in stream_for_re.captures_iter(content) {
        push_or_update(cap[1].to_string(), "String".to_string(), Some("Stream message item payload.".to_string()));
    }
    ...
    for cap in fn_decl_re.captures_iter(content) {
        let doc_str = cap.get(1).or_else(|| cap.get(2)).map(|m| m.as_str().to_string());
        let kind_kw = &cap[3];
        let name_str = &cap[4];
